...
public:
	...
	std::string GetEntPropString(CBaseEntity* entity, PropType proptype, const char* prop, int element = 0);
	 ...
	 * @return Array size or 0 if not an array or if the look up failed.
	 */
	std::size_t GetEntPropArraySize(CBaseEntity* entity, PropType proptype, const char* prop);
...
namespace entityprops
{
	...
	// Converts a string to prop type, used when parsing config files.
	PropType StringToPropType(const char* str);
}
